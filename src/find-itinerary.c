...
void find_itinerary (struct cellule_airport *main_HT[max_Hairport],
        ...
        while (Buffairline != NULL)
        {
            ...
            while (Buffvol != NULL)
            {
                if ((Buffvol->vol.CANCELLED == 0) && (Buffvol->vol.DIVERTED == 0) &&
                    (Buffvol->vol.SCHED_DEP > time) && (Buffvol->vol.DAY == J) && (*limit > 0))
                {
                    ...
                    else
                    {
                        if (Buffvol->vol.SCHED_DEP < Buffvol->vol.SCHED_ARR)
                        {
                            ajout_tete_itinerary (pItinerary, Buffvol->vol, Buffairline->IATA_CODE);
                            if (!(mask & limitON))
                                mask = timeON; // si limit est desacctivée -> time on, limit off
                            else
                                mask = timeON_limitON; // sinon time and limit on
                            find_itinerary (main_HT, pItinerary, Buffvol->vol.DEST_AIR, DEST_AIR, M,
                                            J, Buffvol->vol.SCHED_ARR, limit, mask);
                        }
                    }
                }
...
void init_find_itinerary (struct line_arguments liste, struct cellule_airport *main_HT[max_Hairport])
{
    ...
    if (number_parameter_test (liste, nb_arg_find_itinerary, nb_optionnal_arg_find_itinerary) == 0)
        return; // Test sur le nombre de paramètre
    occ_test++; // 1
    if (lenght_parameter_test (liste, SIZE_acronym - 1, occ_test) == 0)
        return; // Test sur la longueur du paramètre "<port_id1>"
    if (number_parameter_test (liste, nb_arg_find_itinerary, nb_optionnal_arg_find_itinerary) == 0)
        return; // Test sur le nombre de paramètre
    occ_test++; // 2
    if (lenght_parameter_test (liste, SIZE_acronym - 1, occ_test) == 0)
    ...
    {
        occ_test++; // 4
        if (wrong_int_test (liste.arg[occ_test], occ_test) == 0)
            return; // Test sur type avant conversion
